...
4. Use APEX Assistant to generate sample data with prompt: "Generate sample data for all tables"
5. **Replace the generated script** with the complete data model script below:
...
INSERT INTO cs_tickets (ticket_id, ticket_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (3, 'TICKET-20240101-4', 4, 4,   'Credit card payment pending',    'Customer reports payment not updated',                    'Low',     'New',         'Phone',  4);
INSERT INTO cs_tickets (ticket_id, ticket_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
...
We'll create a complete AI-powered ticket summary system by building a modal dialog page that displays formatted ticket summaries with loading indicators, configuring RAG sources for data retrieval, implementing automatic text generation on page load, and adding navigation from the ticket finder page.

*[Navigate to App Builder → Create Page]*

**Demo Steps:**
1. Create Modal Dialog Page (Page 13) named "Ticket Summary"
2. Add hidden page items:
   - P13_TICKET_ID (Hidden)
   - P13_TICKET_SUMMARY (Hidden)
3. Create Dynamic Content region with loading spinner:
   ```sql
   ...
   END IF;
   ```
4. Add Close button with dynamic action to close dialog
5. Configure Page Load dynamic action with "Generate Text with AI":
   - Configuration: Ticket AI Configuration
   - Input Value: Only System Prompt
   - Use Response: P13_TICKET_SUMMARY
6. Add second action to refresh the Dynamic Content region
7. Create RAG Source "Ticket Summary Details" with SQL query:
   ```sql
   ...
   ```
   - Server-side Condition: RETURN :APP_PAGE_ID = 13;
8. Add "Generate Ticket Summary" action to Tickets report (Page 5):
   - Position: Primary Actions
   - Template: Button
   - Label: Generate Ticket Summary
   - Target: Page 13, Set P13_TICKET_ID = &TICKET_ID.
9. Test the complete flow: Click button → Modal opens → AI generates summary → Formatted display
