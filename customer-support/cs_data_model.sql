...
CREATE OR REPLACE TRIGGER cs_cases_biu
    BEFORE INSERT OR UPDATE ON cs_cases
    FOR EACH ROW
BEGIN
    IF INSERTING THEN
        :new.created_at := SYSDATE;
        ...
        IF :new.case_number IS NULL THEN
            :new.case_number := 'CASE-' || TO_CHAR(SYSDATE, 'YYYYMMDD') || '-' || :new.case_id;
        END IF;
    END IF;
    :new.updated_at := SYSDATE;
    :new.updated_by := COALESCE(SYS_CONTEXT('APEX$SESSION','APP_USER'), USER);
...
-- ============================================================================
-- TRIGGER: Auto-create timeline entry when case is created
-- ============================================================================
CREATE OR REPLACE TRIGGER cs_cases_timeline_ai
    AFTER INSERT ON cs_cases
    FOR EACH ROW
BEGIN
    INSERT INTO cs_case_updates (case_id, update_text, updated_by, updated_at)
    VALUES (
        :new.case_id,
        'Case created with priority: ' || :new.priority || ', status: ' || :new.status,
        :new.created_by,
        SYSDATE
    );
END cs_cases_timeline_ai;
/

-- ============================================================================
-- TRIGGER: Auto-create timeline entry when case status or assignment changes
-- ============================================================================
CREATE OR REPLACE TRIGGER cs_cases_timeline_au
    AFTER UPDATE ON cs_cases
    FOR EACH ROW
DECLARE
...
BEGIN
    ...
    IF :new.status != :old.status THEN
        ...
        INSERT INTO cs_case_updates (case_id, update_text, updated_by, updated_at)
        VALUES (:new.case_id, v_update_text, :new.updated_by, SYSDATE);
    END IF;
    
    -- Track assignment changes
    IF NVL(:new.assigned_agent_id, -1) != NVL(:old.assigned_agent_id, -1) THEN
        IF :old.assigned_agent_id IS NULL THEN
            v_update_text := 'Case assigned to agent ID: ' || :new.assigned_agent_id;
        ELSIF :new.assigned_agent_id IS NULL THEN
            v_update_text := 'Case unassigned from agent ID: ' || :old.assigned_agent_id;
        ELSE
            v_update_text := 'Case reassigned from agent ID: ' || :old.assigned_agent_id || 
    ...
    IF :new.priority != :old.priority THEN
        ...
        VALUES (:new.case_id, v_update_text, :new.updated_by, SYSDATE);
    END IF;
END cs_cases_timeline_au;
/

-- ============================================================================
-- TRIGGER: Auto-create SLA record when case is created
-- ============================================================================
CREATE OR REPLACE TRIGGER cs_cases_sla_ai
    AFTER INSERT ON cs_cases
    FOR EACH ROW
DECLARE
...
BEGIN
    ...
    -- Only update SLA status if case is not closed or resolved
    IF v_case_status NOT IN ('Closed', 'Resolved') THEN
...
CREATE OR REPLACE TRIGGER cs_case_updates_biu
BEFORE INSERT OR UPDATE ON cs_case_updates
FOR EACH ROW
BEGIN
...
-- Cases
-- Cases
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (1, 'CASE-20240101-1', 1, 1,   'KYC document not verified',      'Customer submitted KYC docs but not verified',            'Medium',  'New',         'Email',  1);
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (2, 'CASE-20240101-2', 2, 2,   'Unable to access account',       'Corporate login fails with auth error',                   'High',    'In Progress', 'Chat',   2);
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (3, 'CASE-20240101-3', 3, 3,   'Loan account showing blocked',   'Loan EMI paid, but blocked warning in app',               'Critical','Waiting',     'Mobile App', 3);
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (4, 'CASE-20240101-4', 4, 4,   'Credit card payment pending',    'Customer reports payment not updated',                    'Low',     'New',         'Phone',  4);
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (5, 'CASE-20240101-5', 5, 5,   'Fixed deposit wrongly closed',   'FD closed without consent',                               'High',    'Resolved',    'Email',  5);
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (6, 'CASE-20240101-6', 6, 6,   'Closed account showing open',    'Savings account shows open after closure',                'Medium',  'Closed',      'Branch', 6);
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (7, 'CASE-20240101-7', 7, 7,   'Unable to withdraw cash',        'ATM not dispensing, account debited',                     'Critical','Cancelled',   'Phone',    7); 
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (8, 'CASE-20240101-8', 8, 8,   'Loan account missing in app',    'Customer cannot see loan details',                        'Medium',  'New',         'Web',    8);
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (9, 'CASE-20240101-9', 9, 9,   'Credit card limit reduced',      'Customer not informed before reduction',                  'High',    'New',         'Email',  9);
INSERT INTO cs_cases (case_id, case_number, customer_id, account_id, subject, description, priority, status, channel, assigned_agent_id)
VALUES (10,'CASE-20240101-10',10, 10, 'Unable to withdraw - savings',   'Customer unable to withdraw even after sufficient funds', 'Medium',  'In Progress', 'Chat',  10);

-- Case Updates
INSERT INTO cs_case_updates (case_id, update_text, updated_by)
...
INSERT INTO cs_interactions (case_id, channel, message_text, direction, created_by)
